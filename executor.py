...
class ActionExecutor:
    ...
    def __init__(
        ...
        # V25.17:
        # Prefer the ScreenAgent owned by MJBrain.
        # This prevents duplicate ScreenContext instances.
        self.screen_agent = screen_agent

        if self.screen_agent is not None:
            print(
                "MJ V25.17: Shared ScreenAgent attached."
            )
    ...
    def open_any_app(
        ...
        try:
            ...
            print(
                'MJ APP LAUNCHER ERROR:',
                exc,
            )

        # =====================================================
        # WINDOWS REGISTRY APP SCANNER
        # =====================================================
    ...
    def open_target(self, target):
        ...
        try:
            ...
            if target == "chrome":
                ...
                if not opened:
                    return (
                        "Chrome nahi mila aur default browser bhi nahi khula.",
                        "hi",
                        False,
                    )
            ...
            # =====================================================
            # UNIVERSAL APP FALLBACK
            # =====================================================

            app_result = self.open_any_app(
                target
            )
    ...
    def screen_action(self, action="", query="", **kwargs):
        ...
        if self.screen_agent is None:
            try:
                ...
                print(
                    "MJ V25.17: "
                    "Standalone ScreenAgent created."
                )
    ...
    # =========================================================
    # V25.16 SCREEN VERIFY
    # =========================================================

    def _refresh_screen_context(self):
        """
        Refresh ScreenContext after an action.

        This is deliberately defensive so that screen
        verification can never break normal MJ actions.
        """
    ...
    def _screen_changed(
        ...
        """
        Compare screen signatures when available.

        Returns:
            True  = screen changed
            False = no detected change
    ...
    def _verify_screen_action(
        ...
        """
        Verify a screen action without making verification
        mandatory for success.

        Existing ScreenAgent success remains authoritative.
        ScreenContext is used as additional evidence.
        """
        ...
        try:
            message, language, success = result

            if not success:
                return result

            # Observe current state after successful action.
            after = self._refresh_screen_context()

            if after is not None:
                print(
                    "MJ V25.16: "
                    "Screen state refreshed after action."
                )
