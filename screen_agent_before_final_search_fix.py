...
class ScreenAgent:
    ...
    def browser_search_and_type(
        ...
        """
        Activate the existing browser and perform a Google search.
        """
        ...
        try:
            ...
            def enum_callback(hwnd, lparam):
                ...
                if (
                    "google chrome" in lower_title
                    or "microsoft edge" in lower_title
                ):
                    browser_windows.append(
                        (
                            hwnd,
            ...
            chrome_windows = [
                item
                for item in browser_windows
                if "chrome" in item[1].lower()
            ]
            ...
            # -------------------------------------------------
            # Reliable Windows foreground activation
            # -------------------------------------------------
            ...
            for attempt in range(12):
                ...
                try:
                    ...
                    if (
                        current_thread
                        and browser_thread
                        and current_thread != browser_thread
                    ):
                        attached = bool(
            ...
            print(
                "MJ SEARCH: Google search submitted."
            )

            time.sleep(3.0)

            return (
                f"{query} search kar diya.",
                "hi",
                True,
            )
            ...
            # -------------------------------------------------
            # RELIABLE FOREGROUND ACTIVATION
            # Repeat AttachThreadInput on every attempt.
            # -------------------------------------------------
            ...
            for attempt in range(10):
                ...
                try:
                    ...
                    if (
                        current_thread
                        and chrome_thread
                        and current_thread != chrome_thread
                    ):
                        attached = bool(
                            user32.AttachThreadInput(
                                current_thread,
                ...
                if foreground == chrome_hwnd:
                    active = True

                    print(
                        "MJ SEARCH ACTIVATE: SUCCESS attempt",
                        attempt + 1,
                    )
            ...
            if not success:
                return (
                    "Chrome address bar focus nahi ho paya.",
                    "hi",
                    False,
                )
    ...
    def enum_callback(
            ...
            # Attach input queues so Windows permits
            ...
            # Give Windows time to switch.
            time.sleep(0.8)

            # -------------------------------------------------
            # Verify foreground repeatedly
            # -------------------------------------------------
    ...
    def browser_page_summary(
        ...
        """
        Read the current browser screen and keep meaningful
        search-result text while ignoring VS Code/editor noise.
        """
        ...
        for item in items:
            ...
            priority_hits = len(
                priority_words & words
            )

            # Search result should preferably
            ...
            score += topic_hits * 100
            score += priority_hits * 20
            ...
            # Browser content is normally below
            # the top browser/editor chrome.
            y = item.get("y", 0)
