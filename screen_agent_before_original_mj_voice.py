...
class ScreenAgent:
    ...
    def mj_speak(
        ...
        """Speak MJ response using the original/default Windows voice."""
        ...
        try:
            ...
            # -------------------------------------------------
            # ORIGINAL MJ VOICE
            # Do NOT force Zira/David.
            # Windows default voice will be used.
            # -------------------------------------------------
    ...
    def speak_search_result(
        ...
        """Read browser search result aloud."""
    ...
    def open_browser(
        ...
        """
        Open browser URL and dynamically activate Chrome/Edge.
        Does not depend on a hardcoded HWND.
        """
        ...
        try:
            ...
            # =================================================
            # CURRENTLY VISIBLE BROWSER WINDOWS
            # =================================================
            ...
            # =================================================
            # FIND WINDOWS AGAIN AFTER LAUNCH
            # =================================================

            browser_windows = []

            def enum_callback_after(hwnd, lparam):
            ...
            # Prefer the browser that is already visible.
            if browser_windows:

                # Prefer Chrome if available.
                chrome = [
                    x
                    for x in browser_windows
                    if "chrome" in x[1].lower()
                ]

                if chrome:
                    browser_hwnd = chrome[-1][0]
                else:
                    browser_hwnd = browser_windows[-1][0]

            if not browser_hwnd:
                return (
                    "Browser window nahi mila.",
                    "hi",
                    False,
                )
            ...
            activated = user32.SetForegroundWindow(
                browser_hwnd
            )
            ...
            title_buf = ctypes.create_unicode_buffer(
                512
            )
            ...
            if (
                "chrome" not in current_lower
                and "edge" not in current_lower
            ):
                return (
                    "Browser foreground mein nahi aa paya.",
                    "hi",
                    False,
                )
            ...
            success = self.controller.hotkey(
                "ctrl",
                "l",
            )
            ...
            if not success:
                return (
                    "Browser address bar focus nahi ho paya.",
                    "hi",
                    False,
                )
            ...
            success = self.controller.type_text(
                url
            )

            if not success:
                return (
                    "Browser URL type nahi ho paya.",
                    "hi",
                    False,
                )
            ...
            success = self.controller.press(
                "enter"
            )

            if not success:
                return (
                    "Browser URL open nahi ho paya.",
                    "hi",
                    False,
                )
            ...
            print(
                "MJ BROWSER OPEN ERROR:",
                exc,
            )

            return (
                "Browser open nahi ho paya.",
                "hi",
                False,
            )
    ...
    def browser_search_and_type(
        ...
        """
        Activate an existing browser window first,
        then search using Ctrl+L.
        """
        ...
        print(
            "MJ SCREEN AGENT: Browser search:",
            value,
        )
        ...
        try:
            ...
            # =================================================
            # FIND VISIBLE CHROME / EDGE WINDOWS
            # =================================================
            ...
            if chrome_windows:
                browser_hwnd = chrome_windows[-1][0]
            else:
                browser_hwnd = browser_windows[-1][0]
            ...
            if (
                ...
                return (
                    "Browser foreground mein nahi hai.",
                    "hi",
                    False,
                )
            ...
            print(
                "MJ SCREEN AGENT: Focusing browser address bar"
            )
            ...
            if not success:
                return (
                    "Browser search box focus nahi ho paya.",
                    "hi",
                    False,
                )
            ...
            print(
                "MJ SCREEN AGENT: Search typed:",
                value,
            )

            return (
                f"'{value}' search mein type kar diya.",
                "hi",
                True,
            )
            ...
            print(
                "MJ BROWSER SEARCH ERROR:",
                exc,
            )

            return (
                "Browser search nahi ho paya.",
                "hi",
                False,
            )
    ...
    def observe_browser_page(
        ...
        """Read useful visible browser text without clicking."""
        ...
        try:
            ...
            print("MJ BROWSER OBSERVER ERROR:", exc)
            return []
        ...
        for item in data:
            ...
            if re.search(
                r"\.(py|pyc|json|txt|bat|log|ini|cfg)$",
                lower,
            ):
                continue
    ...
    def browser_page_summary(
        ...
        """
        Intelligent browser-result filtering.

        OCR mein VS Code / PowerShell / terminal text aa sakta hai.
        Isliye sirf topic-related meaningful browser text ko priority
        deta hai.
        """
        ...
        topic_words = set(
            topic_normalized.split()
        )

        # ---------------------------------------------------------
        # UI / IDE / TERMINAL WORDS
        # ---------------------------------------------------------
        ...
        # ---------------------------------------------------------
        # WORDS THAT INDICATE REAL CRICKET / PERSON INFORMATION
        # ---------------------------------------------------------
        ...
        for item in items:
            ...
            # Single meaningless OCR fragment
            if len(words) <= 1:
                continue
            ...
            # For "Tilak Varma", require at least one strong
            # relation unless the text is clearly a useful headline.
            if (
                topic_match == 0
                and useful_match == 0
            ):
            ...
            score += topic_match * 100
            score += useful_match * 20

            # Prefer actual sentences/headlines.
            if len(words) >= 4:
                score += 20
            ...
            # Very short OCR is less reliable.
            if len(text) < 10:
                score -= 20
    ...
    def ask_to_open_browser_result(
        ...
        """
        Browser search result ko natural confirmation mein convert karta hai.
        Random OCR fragments ko bolne se rokta hai.
        """
        ...
        if not items:
            return (
                "Mujhe abhi koi clear relevant result nahi mila. "
                "Kya main search ko aur refine karun?",
                "hi",
                False,
            )
        ...
        for item in items:
            ...
            # ---------------------------------------------
            # Must contain topic OR meaningful news word
            # ---------------------------------------------

            topic_words = set(
                self.normalize(topic).split()
            )
            ...
            if not has_topic and not has_meaning:
                continue
        ...
        if not useful:
            return (
                f"{topic} ke search results mil gaye hain, "
                "lekin abhi clear information nahi dikh rahi. "
                "Kya main aur search karun?",
                "hi",
                False,
            )
        ...
        if len(useful) == 1:
            ...
            message = (
                ...
                f"'{useful[0]}' relevant lag raha hai. "
                "Kya main ise khol kar details dekhun?"
            )
            ...
            message = (
                ...
                + ", ".join(useful)
                + " jaise relevant results dikh rahe hain. "
                "Kya main sabse relevant result khol kar "
                "details dekhun?"
            )
...
def autonomous_screen_agent(controller, goal, planner=None,
    ...
    for _ in range(state.max_steps):
        ...
        if any(k in goal_lower for k in (
            "search", "find", "dhoond", "dhund", "à¤¢à¥‚à¤‚à¤¢"
        )):
            for candidate in (
                "search box",
                "search",
